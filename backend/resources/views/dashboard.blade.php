...
<div class="wrap">
    ...
    <div class="layout">
        <aside class="side-menu" aria-label="Dashboard pages">
            ...
            <nav class="nav" aria-label="Dashboard sections">
                ...
                <a class="{{ $activePage === 'audits' ? 'active' : '' }}" href="{{ route('dashboard', ['page' => 'audits']) }}">Audit Logs</a>
            </nav>
        </aside>
    ...
    <div id="shops" class="section">
        ...
        <div class="table-wrap">
            <table>
                ...
                @php
                    $registrationPayment = $shop->registrationFeePayment;
                @endphp
                <tr>
    ...
    <div id="audits" class="section">
        <div class="section-head">
            <h2>Audit Logs</h2>
            <form class="tools" method="get" action="{{ route('dashboard', ['page' => 'audits']) }}">
                <input name="audits_q" value="{{ $filters['audits_q'] }}" placeholder="Search audit logs">
                <select name="audits_per_page">@foreach($pageOptions as $option)<option value="{{ $option }}" @selected($perPage['audits_per_page'] === $option)>{{ $option }} per page</option>@endforeach</select>
            </form>
        </div>
        <div class="table-wrap">
            <table>
                <tr><th>When</th><th>Event</th><th>Actor</th><th>Action</th><th>Target</th><th>Details</th><th>Status</th></tr>
                @forelse($auditLogs as $auditLog)
                @php
                    $oldValues = $auditLog->old_values ?? [];
                    $newValues = $auditLog->new_values ?? [];
                    $changedKeys = collect(array_unique(array_merge(array_keys($oldValues), array_keys($newValues))))->take(8)->join(', ');
                    $payloadKeys = collect(array_keys((array) data_get($auditLog->metadata, 'request_payload', [])))->take(8)->join(', ');
                    $target = $auditLog->auditable_type ? class_basename($auditLog->auditable_type).' #'.$auditLog->auditable_id : '-';
                    $isFailed = $auditLog->response_status && $auditLog->response_status >= 400;
                @endphp
                <tr>
                    <td>{{ $auditLog->created_at }}<div class="muted">{{ $auditLog->request_id }}</div></td>
                    <td><span class="pill">{{ $auditLog->source }}</span><div class="muted">{{ $auditLog->event }}</div></td>
                    <td>{{ $auditLog->actor_name ?? 'System' }}<div class="muted">@if($auditLog->actor_email){{ $auditLog->actor_email }}<br>@endif{{ $auditLog->actor_role ?? 'system' }} @if($auditLog->actor_id)#{{ $auditLog->actor_id }}@endif</div></td>
                    <td>{{ $auditLog->action ?? '-' }}<div class="muted">@if($auditLog->method){{ $auditLog->method }} @endif{{ $auditLog->path ?? $auditLog->route_name }}</div></td>
                    <td>{{ $target }}<div class="muted">{{ $auditLog->route_name }}</div></td>
                    <td>@if($changedKeys)Changed: {{ $changedKeys }}@elseif($payloadKeys)Payload: {{ $payloadKeys }}@else<span class="muted">No changed fields captured</span>@endif</td>
                    <td class="{{ $isFailed ? 'blocked' : 'status' }}">{{ $auditLog->response_status ?? 'ok' }}<div class="muted">{{ $auditLog->duration_ms ? $auditLog->duration_ms.'ms' : '' }}</div></td>
                </tr>
                @empty
        ...
        <div class="pagination"><span>{{ $auditLogs->total() }} result(s)</span>{{ $auditLogs->links() }}</div>
    </div>
    @endif
        </main>
    </div>
