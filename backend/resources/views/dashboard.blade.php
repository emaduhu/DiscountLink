...
<div class="wrap">
    ...
    <div id="orders" class="section"><h2>Recent Orders</h2><div class="table-wrap"><table><tr><th>Ref</th><th>Buyer</th><th>Seller</th><th>Shop</th><th>Status</th><th>Total</th><th>Deliverer</th></tr>@foreach($orders as $order)<tr><td>{{ $order->reference }}</td><td>{{ $order->buyer?->name }}</td><td>{{ $order->seller?->name }}</td><td>{{ $order->shop?->name }}</td><td class="status">{{ $order->status }}</td><td>{{ number_format($order->grand_total,2) }}<div class="muted">Service {{ number_format($order->service_fee_total ?? 0,2) }} @ {{ number_format($order->service_fee_rate ?? 0,2) }}%</div></td><td>{{ $order->deliveryAssignment?->deliverer?->name ?? 'Unassigned' }}</td></tr>@endforeach</table></div></div>
    ...
    <div id="payments" class="section"><h2>Payments and Disbursements</h2><div class="table-wrap"><table><tr><th>Order / Shop</th><th>Type</th><th>Status</th><th>Amount</th><th>Phone</th><th>Provider Ref</th></tr>@foreach($payments as $payment)<tr><td>{{ $payment->order?->reference ?? $payment->shop?->name ?? '-' }}@if($payment->shop)<div class="muted">Shop registration</div>@endif</td><td>{{ $payment->type }}</td><td class="{{ $payment->status === 'failed' ? 'blocked' : 'status' }}">{{ $payment->status }}</td><td>{{ number_format($payment->amount,2) }}</td><td>{{ $payment->phone }}</td><td>{{ $payment->provider_reference }}</td></tr>@endforeach</table></div></div>
</div>
</body>
</html>
